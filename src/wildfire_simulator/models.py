...
class MK_UNet_Regression(nn.Module):
    """
    Regression variant of MK_UNet.
    Outputs a feature map with `out_channels` (default 1) without softmax.
    Set `final_activation` to 'relu', 'sigmoid', None, etc., depending on the regression task.
    """
    def __init__(self, 
                 out_channels=1, 
                 in_channels=3, 
        ...
        # ---------- Attention Gates ----------
        self.AG1 = GroupedAttentionGate(channels[3], channels[3], channels[3]//2, 
                                        kernel_size=gag_kernel, groups=channels[3]//2)
        self.AG2 = GroupedAttentionGate(channels[2], channels[2], channels[2]//2, 
                                        kernel_size=gag_kernel, groups=channels[2]//2)
        self.AG3 = GroupedAttentionGate(channels[1], channels[1], channels[1]//2, 
                                        kernel_size=gag_kernel, groups=channels[1]//2)
        self.AG4 = GroupedAttentionGate(channels[0], channels[0], channels[0]//2, 
        ...
        # Final activation (e.g., 'relu', 'sigmoid', None)
        if final_activation is None or final_activation.lower() == 'none':
            self.head_act = nn.Identity()
        else:
            self.head_act = act_layer(final_activation, inplace=True)

    def forward(self, x):
        # Convert single-channel inputs to 3-channel (if needed)
        if x.shape[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        ...
        t4 = out
        out = F.max_pool2d(self.encoder5(out), 2, 2)   # bottleneck
        ...
        return [out]   # returned as list for compatibility with existing pipelines
